...
public class UserInfo extends Server {
    ...
    public JSONArray getUserCredentials(String email) {
        ...
        try {
            ...
            if (getResultSet().next()) {
                ...
                jo.put("rateText", "(Betyg " + getResultSet().getDouble("rate") + " av totalt " + getResultSet().getInt("votes") + " röster)");
                jo.put("rate", getResultSet().getDouble("rate"));
    ...
    public JSONArray getUserCredentials(int id) {
        JSONArray ja = new JSONArray();
        JSONObject jo = new JSONObject();

        try {
            String args = "SELECT * FROM Users WHERE userid = " + id + ";";
            setStatement(getConn().createStatement());
            setResultSet(getStatement().executeQuery(args));
